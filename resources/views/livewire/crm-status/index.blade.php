    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="CrmStatus" format="csv" />
                <livewire:excel-export model="CrmStatus" format="xlsx" />
                <livewire:excel-export model="CrmStatus" format="pdf" />
            @endif


            @can('crm_status_create')
                <x-csv-import route="{{ route('admin.crm-statuses.csv.store') }}" />
            @endcan
    ...
    <div class="overflow-hidden">
        <div class="overflow-x-auto">
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($crmStatuses as $crmStatus)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $crmStatus->id }}" wire:model="selected">
                            </td>
                            <td>
                                {{ $crmStatus->id }}
                            </td>
